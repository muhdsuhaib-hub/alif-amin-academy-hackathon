...
export default function Auth() {
  ...
  const handleGoogleLogin = () => {
    ...
    window.location.href = `https://accounts.google.com/o/oauth2/v2/auth?client_id=${clientId}&redirect_uri=${encodeURIComponent(redirectUri)}&response_type=code&scope=${encodeURIComponent('email profile')}&access_type=offline&prompt=consent`;
  };
  ...
  const handleEmailRegister = async (e) => {
    ...
    try {
      const response = await fetch(`${API}/auth/register`, { method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email: formData.email, password: formData.password, full_name: formData.fullName, phone: `${formData.countryCode}${formData.phoneNumber}`, role: 'student', schedule_preference: formData.schedulePreference, reading_level: formData.readingLevel, goals: formData.goals }) });
      const data = await response.json();
