    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#fbfaf6]">
        <div className="text-center">
          ...
          <p className="text-body text-ink-secondary font-['Bricolage_Grotesque']">Setting up your experience...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[#fbfaf6]">
      <div className="w-full max-w-2xl mx-auto px-6 py-12">
        <button onClick={handleBack} className="flex items-center gap-2 mb-12 text-small font-medium text-ink-tertiary hover:text-[#0f2e24] transition-colors font-['Bricolage_Grotesque']">
          <ArrowLeft className="w-3.5 h-3.5" /> Back
        </button>

        {step === 0 && (
          <div className="text-center mb-12">
            <p className="text-h3 font-semibold text-[#0f2e24] mb-2 font-['Bricolage_Grotesque']">Absolutely no payment required to begin.</p>
            <p className="text-body text-ink-secondary font-['Bricolage_Grotesque']">Just a few questions to personalise your experience.</p>
          </div>
        )}
        ...
        <div className="flex gap-2 mb-2">
          {questions.map((_, i) => (
            <div key={i} className={`flex-1 h-1 rounded-full transition-colors duration-300 ${i <= step ? 'bg-[#0f2e24]' : 'bg-surface-muted'}`} />
          ))}
        </div>
        <p className="text-caption text-ink-tertiary mb-8 font-['Bricolage_Grotesque']">Question {step + 1} of {questions.length}</p>

        <AnimatePresence mode="wait">
          <motion.div key={step} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.25 }}>
            <h2 className="text-h1 text-[#0f2e24] mb-8 font-['Libre_Baskerville']">{current.title}</h2>
            <div className="space-y-3">
              {current.options.map((opt) => (
                <motion.button
                  ...
                  whileTap={{ scale: 0.98 }}
                  className="w-full flex items-center justify-between p-5 rounded-lg border border-ink-faint/30 bg-white text-left hover:border-[#0f2e24] hover:shadow-md transition-all duration-200"
                >
                  <div>
                    <p className="text-body font-semibold text-[#0f2e24] font-['Bricolage_Grotesque']">{opt.label}</p>
                    <p className="text-small text-ink-secondary font-['Bricolage_Grotesque']">{opt.subtitle}</p>
                  </div>
